...
export default{
  ...
  methods: {
    ...
    async fetchPosts() {
      try{
        const responce = await api().get('posts/' + this.myCategory);
        this.totalPages = Math.ceil(responce.data.length / this.limit)
        this.page = Math.ceil(responce.data.length / this.limit)
        // this.posts = responce.data.splice(0, this.totalPages);
